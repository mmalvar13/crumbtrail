...
try {
	...
	$event = filter_input(INPUT_GET, "event", FILTER_VALIDATE_STRING);
	...
	if($method === "GET") {
		...
		if(empty($id) === false && empty($eventTruckId) === false) {
			...
			} elseif((empty($eventLocationLat)) === false && (empty($eventLocationLng) === false){
				$point = new Point(null, $eventLocationLat, $eventLocationLng);
			$event = Event::getEventByEventLocation($pdo, $point);
			if($event != null) {
				$reply->data = $event;
			}
		} elseif
