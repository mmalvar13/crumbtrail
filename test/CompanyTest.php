...
class CompanyTest extends CrumbTrailTest {
	...
	public function testGetCompanyByCompanyAccountCreatorId() {
		...
		// Grab the data from mySQL and enforce the fields match our expectations.
		$results = Company::getCompanyByCompanyAccountCreatorId($this->getPDO(), $company->getCompanyAccountCreatorId());
		$this->assertEquals($numRows + 1, $this->getConnection()->getRowCount("company"));
		$this->assertCount(1, $results);
		$this->assertContainsOnlyInstancesOf("Edu\\Cnm\\CrumbTrail\\Company", $results);
	...
	public function testGetCompanyByCompanyMenuText() {
		...
		// Grab the data from mySQL and enforce the fields match our expectations.
		$results = Company::getCompanyByCompanyMenuText($this->getPDO(), $company->getCompanyMenuText());
		$this->assertEquals($numRows + 1, $this->getConnection()->getRowCount("company"));
		$this->assertCount(1, $results);
		$this->assertContainsOnlyInstancesOf("Edu\\Cnm\\CrumbTrail\\Company", $results);
